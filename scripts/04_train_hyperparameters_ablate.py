...
ablation = str(sys.argv[2])
if (ablation != 'erm') and (ablation != 'fbm'):
...
tensorboard_dir = f'{engram_dir}tensorboard/{ablation}_ablation/'
...
def train_and_eval(noise_type, snr_level):
    ...
    if SAME_PARAM:
        if ablation == 'erm':
            ...
            optimizer = torch.optim.Adam([
                {'params': [hyperparams[0],hyperparams[2]], 'lr':0.01*LR_SCALING},
