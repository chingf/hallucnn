...
def save_activations(pnet, dset, hdf5_path):
    ...
    with h5py.File(hdf5_path, 'x') as f_out:
        ...
        for idx in range(dset.n_data):
            ...
            for timestep in range(n_timesteps):
                for layer in layers:
                    data_dict[f'{layer}_{timestep}_activations'][idx] =                         activations[timestep][layer]
                data_dict[f'{timestep}_logits'][idx] =                     logits[timestep]
            ...
            for timestep in range(n_timesteps):
                for layer in layers:
                    data_dict[f'{layer}_{timestep}_clean_activations'][idx] =                         activations[timestep][layer]
                data_dict[f'{timestep}_clean_logits'][idx] =                     logits[timestep]
